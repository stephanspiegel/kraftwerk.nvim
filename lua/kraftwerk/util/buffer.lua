local function get_positions_for_range(start_marker, end_marker)
    -- get position of start of marked area
    local _, csrow, cscol, _ = unpack(vim.fn.getpos(start_marker))
    -- end of marked area is the cursor position
...
local function get_visual_selection_range()
    ...
    local start_row, start_column, end_row, end_column = get_positions_for_range(start_marker, end_marker)
    if mode == "V" then
        start_column = 1
        end_column = 2^31 - 1
    end
...
local function get_visual_selection()
    local start_row, start_column, end_row, end_column = get_visual_selection_range()
